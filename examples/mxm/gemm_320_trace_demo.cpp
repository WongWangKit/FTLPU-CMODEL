...
namespace {
    ...
    {
        std::int32_t sum = 0;
        for (std::size_t k = 0; k < kComputeCycles; ++k) {
    ...
    bool verify_gemm_results(const ftlpu::MxmGemmEngine* gemm)
    {
        bool all_match = true;
        std::int32_t first_mismatch_expected = 0;
        std::int32_t first_mismatch_actual = 0;
        std::size_t first_mismatch_row = 0;
        std::size_t first_mismatch_col = 0;
        bool found_mismatch = false;

        for (std::size_t row = 0; row < ftlpu::hw::kMxmRows; ++row) {
            const auto& result_row = gemm->accumulator_row(row);
            for (std::size_t col = 0; col < ftlpu::hw::kMxmColumns; ++col) {
                const auto expected = expected_result(row, col);
                if (result_row[col] != expected) {
                    if (!found_mismatch) {
                        first_mismatch_row = row;
                        first_mismatch_col = col;
                        first_mismatch_expected = expected;
                        first_mismatch_actual = result_row[col];
                        found_mismatch = true;
                    }
                    all_match = false;
                }
        ...
        if (!all_match) {
            std::cerr << "GEMM MISMATCH at row=" << first_mismatch_row
                << " col=" << first_mismatch_col
                << " expected=" << first_mismatch_expected
                << " actual=" << first_mismatch_actual << "\n";
            std::cerr << "... " << (320 * 320) << " total entries checked\n";
            return false;
        }

        std::cout << "GEMM golden verification PASSED: "
            << ftlpu::hw::kMxmRows << "x" << ftlpu::hw::kMxmColumns
            << " = " << (ftlpu::hw::kMxmRows * ftlpu::hw::kMxmColumns)
            << " entries all correct\n";
        return true;
    }
    ...
    mxm_log << "compute A[320,320] x B[320,320], Compute is issued every cycle for " << kComputeCycles
        ...
        return verify_gemm_results(gemm.get()) ? 0 : 1;
    }
