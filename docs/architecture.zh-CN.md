...
每个物理 `8 x 8` supercell 按 15 拍菱形波前执行。局部第 `s` 拍只执行对应
`row+column` 对角线上的 MAC；activation 向东传播，FP32 partial sum 向北传播。
15 拍表示单个 token 的首尾延迟，不是启动间隔：流水线每拍都能接收一个新 token
（`II=1`），最多同时保留 15 个在途 wave。

两套完整的 `8 x 8` weight buffer 构成乒乓对。选定 supercell 的整块权重可以并行
装载；Compute 发射后，在途 wave 的整个生命周期都锁定其选择的 buffer。非活动
buffer 可以在不发生 stream-register 冲突的前提下与当前计算重叠装载，但活动
buffer 不允许被覆盖。
...
`DecodeStreamCompute` 复用同一套 15 级物理 supercell 波前。`Native4x4` 在外层
4×4 阵列上用 7 拍启动 16 个 supercell，内外流水重叠后的总波前为
`7 + 15 - 1 = 21` 拍；`Linear1x16` 每拍启动一个 supercell，因此为
`16 + 15 - 1 = 30` 拍。Decode activation 的两套乒乓 buffer 同样禁止覆盖在途数据。
...
统一黑盒系统甘特图由以下环境变量独立控制：

- `FTLPU_GANTT=off|chip|superlane|full`（默认推荐压缩的 `chip`）；
- `FTLPU_GANTT_FORMAT=csv|html|both`；
- `FTLPU_FULL_INTEGRATION=1` 让完整 Prefill+Decode 层测试展开全部
  Attention hidden tile。

`chip` 只记录资源忙区间；`superlane` 保留 VXM Superlane 计数；`full`
继续展开到各 VXM ALU。采集器是只读 timing observer，不驱动数据或模块
状态，并会把各 phase 的局部周期复位自动拼成单调递增的芯片级时间轴。
