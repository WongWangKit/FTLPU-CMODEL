...
- 每个 slice column 有 20 个 tile-local SRAM block；
- 每个 tile-local SRAM block 有两个 bank；
- 每个 bank 为 4096 word x 16 byte；
- 完整 slice 容量为 20 x 2 x 4096 x 16 byte = 2.5 MiB。
...
- `MxmArray`：20 x 20 supercell grid；
...
buffer 向 east 移动一 column，新 block 进入 column 0。为了最终按 column 0..19
得到预期顺序，MEM 以反序读取 weight column block：先读 19，再读 18，直到 0。
...
完整 320 x 320 weight matrix 包含 20 个 column block。当前测试向一个 buffer
连续发射 20 个 `IW` pulse，同时另一个 buffer 仍可供 in-flight compute 使用。
...
MEM east handoff stream。当 tile row 19 的 contribution 完成一个 result column
...
- `VxmSlice`：20 个 superlane/tile，指令南到北流动。
...
它构造 160 x 320 activation matrix 和三个 640 x 320 weight matrix：
...
- Gate weight：等效 `320 x 640`，分两个 320-column pass 加载；
- Up weight：等效 `320 x 640`，分两个 320-column pass 加载；
- Hidden：`160 x 640`，int8；
- Down weight：`640 x 320`，拆分到两个 MXM；
...
为 seed。Pass 1 reduction 全部 160 个 key position。Pass 2/3 按 `key % 4`
将 key 分到四组互不相交的四-slice MEM group 和四条 VXM pipeline。每个 pass-2
feedback `Add` reduction 40 个 value，再用三条 `Add` 合并四个 partial sum。
...
MXM0 执行 QK GEMM 时，MXM1 使用第二 weight buffer 保存 Wv，并从独立 activation
stream 消费 X。其 V int32 output 与 QK score stream 一同到达 VXM。ALU0..2
执行 softmax pass 1，同时 ALU3..5 独立将 V requantize 为 int8 并写回 MEM。
测试验证 Q、K、V、QK score、softmax maximum/sum 和全部最终 softmax byte。

最终 attention phase 也由 ICU 调度。MEM 在 `E1` 读取保存的 `160 x 320` V；
SXM 捕获十个 16-row block，Permute 对每个 hidden-column block 发射 16 条
weight stream（`E16..E31`）。由于 MXM `IW` 每次把新 block 移入 column 0，
这些 block 按 hidden-block 反序发射，20 cycle 后 MXM1 buffer 0 被 V 完整替换。

MEM 随后在每个 16-key block 内按反序 lane 读取四个条带化 softmax column。
SXM Transpose+Permute 恢复 key order，每 cycle 发射一条 zero-padded 320-byte
query row。MXM1 计算 160 行 `softmax * V`；`W0..W3` 将 little-endian int32
output 送回四个 MEM slice。测试将完整 MXM weight buffer、所有 MXM output
block 和最终 `160 x 320` SRAM matrix 与 golden data 比较。

`sxm_attention_transpose_test` 验证该 GEMM 前所需的 block algorithm。它将 16
个 key column 对齐到 16 条 stream，对 `16 x 16` block 的 `10 x 10` grid 执行
100 条真实 `Transpose sg16`，暂存生成的 row chunk，再用非 identity
`Permute320` map 将每个 160-element query row 组装成 zero-padded 320-lane
MXM activation。测试检查全部 25,600 个 value 和全部 160 个 padding lane。

`sxm_softmax_v_test` 验证 `seq_len=160` 的完整 stream-register 路径。测试代码
只初始化 SRAM 和 ICU queue。ICU MEM Read 通过 `E16..E31` 移动 V，MXM1 同时
接收连续 20 个 `IW` pulse；另一条 MEM Read queue 在 `E1` 移动 softmax column。
由于 MEM instruction 向北传播，SXM 为每个 tile 独立收集 16 个 phase；Transpose
连续发射 `160 + 20 - 1` cycle，覆盖完整 diagonal wavefront。每个局部
`16 x 16` block 仍为 16-cycle latency。

连续 Permute 先捕获本地 Transpose segment，再组装每个 query 对应的十个 key
...
Compute 所需 tile skew 每 cycle 发射一条 zero-padded 320-byte query row。

MXM1 消费连续 160 行 transposed row，通过 `W0..W3` 输出 int32 result。四条
MEM Write queue 将四个 little-endian result byte 存入相邻 MEM slice。测试从
SRAM 还原所有 result，与 scalar golden model 比较完整 `160 x 320` matrix，
...
- MXM0 的 20-cycle `IW` window 完成后 MXM1 立即开始 `IW`。当前 FFN schedule
  中第二个 gate/up pass 使用 `MXM0: cycles 38..57` 和
  `MXM1: cycles 58..77`；
...
- 每个 pass 反序读取 20 个 column block，因为 `IW` 会将每行所选 weight buffer
  向 east 移动。
