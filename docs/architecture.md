...
Every physical `8 x 8` supercell executes a 15-cycle diagonal wavefront. At
local stage `s`, only MACs on the corresponding `row+column` diagonal execute;
activations advance east and FP32 partial sums advance north. Fifteen cycles is
the latency of one token, not the initiation interval: the pipeline accepts one
new token every cycle (`II=1`) and holds up to fifteen waves in flight.

Within one tile, output lane `c` completes at local stage `7+c`. A triangular
set of 28 FP32 skew registers delays lanes by `7,6,...,0` cycles. Stage 14
therefore releases one flat eight-lane vector to the tile-local SXM interface.
The timing snapshot reports both individual deskew writes and completed flat
vectors; the unified Gantt renders this as `MXM n Lane Deskew`.

The two complete `8 x 8` weight buffers form a ping-pong pair. All weights in a
selected supercell block may be loaded in parallel. Compute locks its selected
buffer for the lifetime of every in-flight wave; the inactive buffer may load
concurrently, subject to stream-register collision rules, but an active buffer
...
`DecodeStreamCompute` uses the same 15-stage physical supercell wavefront.
`Native4x4` launches the sixteen supercells on a seven-cycle outer diamond, so
the overlapped array wave is `7 + 15 - 1 = 21` cycles. `Linear1x16` launches one
of the sixteen supercells per cycle and therefore takes `16 + 15 - 1 = 30`
cycles. Decode activation ping-pong buffers obey the same no-overwrite rule.
...
Each SXM has four independent tile-local datapaths. FP16/BF16 low and high
bytes are modeled as two planes. After the MXM lane deskew, one input beat is
one flat eight-lane row. Eight consecutive beats fill an `8 x 8` capture bank.

The capture bank and transpose output bank have fixed roles; they do not
ping-pong. On the eighth beat, fixed transpose wiring loads all 64 elements
into the output bank in one register stage. During the following eight cycles,
the output bank emits one transposed row per cycle while the capture bank
collects the next block. Blocks therefore start at `II=8` with no steady-state
bubble. The timing snapshot reports the one-cycle capture-to-output-bank load.

Permute adds one vector register stage and rearranges complete blocks across
four superlanes/32 lanes. If the downstream path cannot sustain one row per
cycle, backpressure must stop the next capture block before its eighth row.
...
```text
A[128,576] bf16 x W[576,1536] int8 -> C[128,1536] fp32
```

Weights use symmetric per-output-column W8 scales. `MxmDequantInstruction`
and `IWColumn` feed the fixed MXM weight-entry dequantizer, so VXM is not part
of Projection. MXM0/1 compute adjacent output blocks and their local
accumulators sum 18 K tiles. `AccumulatorRead` sends final FP32 rows through
the stream registers to MEM. All 196,608 outputs are read only from MEM and
compared against a BF16-aware scalar golden model.
...
The common black-box system Gantt collector is controlled independently:

- `FTLPU_GANTT=off|chip|superlane|full` (`chip` is the compact default);
- `FTLPU_GANTT_FORMAT=csv|html|both`;
- `FTLPU_FULL_INTEGRATION=1` expands every Attention hidden tile in the
  full Prefill+Decode layer test.

`chip` records busy intervals only. `superlane` retains VXM Superlane counts,
and `full` adds individual VXM ALU rows. The collector is a read-only timing
observer: it never drives data or module state, and it automatically converts
phase-local cycle resets into one monotonic chip-level timeline.
