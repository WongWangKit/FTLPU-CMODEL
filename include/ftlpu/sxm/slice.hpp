...
class SxmSlice {
public:
    // Physical transpose storage supports at most FP16's two byte planes.
    // kTransposeBytePlanes remains the legacy FP16 name used by callers.
...
private:
    ...
    struct TransposeBank {
        ...
        std::array<std::size_t, hw::kTileRows> byte_planes{};
    ...
    {
        const auto planes = instruction.byte_planes;
        return instruction.opcode == SxmOpcode::Transpose
            && ((instruction.src_streams.size() == planes
                    && instruction.dst_streams.size() == planes)
                || (instruction.src_streams.size()
                        == planes * hw::kLanesPerTile
        ...
        if (!all_valid) return;
            ...
            if (parallel_input) {
                for (std::size_t row = 0; row < hw::kLanesPerTile; ++row) {
                    for (std::size_t plane = 0; plane < planes; ++plane) {
                        const auto source = StreamId::from_packed(
                            instruction.src_streams[row * planes + plane].stream);
                        const auto input = fabric.segment(input_column, tile, source);
                        fabric.consume_segment(
                            input_column, tile, source, "SXM parallel byte-plane Transpose");
                ...
                for (std::size_t plane = 0; plane < planes; ++plane) {
                    ...
                    const auto input = fabric.segment(input_column, tile, source);
                    fabric.consume_segment(
                        input_column, tile, source, "SXM serial byte-plane Transpose");
                    for (std::size_t lane = 0; lane < hw::kLanesPerTile; ++lane) {
                        bank.block[plane][lane][tile][row] = input[lane].data;
                    }
                }
        ...
        std::array<bool, hw::kTileRows>& processed_tiles)
    {
        ...
        for (std::size_t destination_tile = 0;
            ...
            if (bank.byte_planes[source_tile] != planes) {
                throw std::logic_error(
                    "SXM Permute byte-plane mode does not match the ready Transpose block");
            }
                ...
                for (std::size_t row = 0; row < hw::kLanesPerTile; ++row) {
                    for (std::size_t plane = 0; plane < planes; ++plane) {
                        stage_block_segment(
                            ...
                            instruction.dst_streams[row * planes + plane],
