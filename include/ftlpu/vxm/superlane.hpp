...
// One shared control unit feeds every lane in a Superlane. Each ALU stage
...
public:
    ...
    std::optional<VxmLaneAluInstruction> current_instruction(
        std::size_t stage) const
    {
        check_stage(stage);
        return current_[stage]
            ? std::optional<VxmLaneAluInstruction> {
...
public:
        ...
        Hemisphere hemisphere) const noexcept
    {
        for (const auto& lane : lanes_) {
            if (!lane.has_stream_inputs(hemisphere)) {
                return false;
            }
        }
        return true;
    ...
    void tick()
    {
        ...
        for (std::size_t item = 0; item < count; ++item) {
            ...
            result.hemisphere = lanes_[0].outputs()[item].hemisphere;
            for (std::size_t lane = 0; lane < kLaneCount; ++lane) {
                const auto& lane_output = lanes_[lane].outputs()[item];
                if (lane_output.stream != result.stream
                    || lane_output.byte_count != result.byte_count
                    || lane_output.hemisphere != result.hemisphere) {
    ...
    std::optional<VxmLaneAluInstruction> current_instruction(
        std::size_t alu) const
    {
        return instruction_control_.current_instruction(
            alu);
    }
