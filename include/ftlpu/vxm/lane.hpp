...
struct VxmLaneAluInstruction {
    ...
    // Capture this operation's completed value in the small C1/C3 scalar
    // register.  Unlike accumulator_write this is a one-shot result write and
    // does not read or initialize an accumulation recurrence.
    bool local_scalar_write{false};
};
...
public:
        ...
        const ExecutionMask& next_feedback_capture = ExecutionMask{})
    {
        ...
        for (std::size_t stage = 0; stage < kAluCount; ++stage) {
            ...
            if (instruction.local_scalar_write) {
                accumulators_[stage] = completed->value;
            }
...
private:
        ...
        if (instruction.local_scalar_write) {
            if (instruction.accumulator_write
                ...
                throw std::invalid_argument(
                    "only C1/C3 may capture a one-shot local scalar result");
            }
        }
        if (instruction.accumulator_write) {
                            ...
                            bool rhs_port) const
    {
        ...
        if (operand.kind == VxmLaneOperandKind::StreamBFloat16) {
            const auto bits = static_cast<std::uint16_t>(bytes[0])
                | (static_cast<std::uint16_t>(bytes[1]) << 8);
            return VxmDataFormat::bf16_bits_to_float(bits) * operand.scale;
        }
    ...
    void emit_output(std::size_t stage, const VxmLaneAluInstruction& instruction, float result)
    {
        ...
        case VxmCastTarget::BFloat16: {
            const auto bits = VxmDataFormat::float_to_bf16_bits(result);
            output.bytes[0] = static_cast<std::uint8_t>(bits);
            output.bytes[1] = static_cast<std::uint8_t>(bits >> 8);
            output.byte_count = 2;
            break;
