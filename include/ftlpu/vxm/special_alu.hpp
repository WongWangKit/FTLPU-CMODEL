...
public:
    ...
    VxmSpecialAlu()
    {
        // Model the architectural power-on LUT image.  Workloads may still
        // reprogram a bank, but autonomous ProgramImage execution must not
        // depend on an out-of-band host call before Exp/Reciprocal/Rsqrt.
        constexpr std::size_t kExpEntries = 256;
        constexpr std::size_t kReciprocalEntries = 256;
        constexpr std::size_t kRsqrtEntries = 512;
        constexpr float kLn2 = 0.6931471805599453f;
        configure_lut(
            VxmSpecialAluOpcode::Exp,
            {-kLn2 / 2.0f,
             kLn2 / static_cast<float>(
                 kExpEntries)},
            make_table(
                -kLn2 / 2.0f,
                kLn2 / static_cast<float>(
    ...
    // Configure the SRAM-backed lookup tables.
    ...
    VxmLutLookupRequest make_lookup(VxmSpecialAluOpcode opcode, float input) const
    {
        ...
        if (opcode == VxmSpecialAluOpcode::Exp) {
            ...
            constexpr float kLn2 = 0.6931471805599453f;
            const auto exponent =
                static_cast<int>(std::nearbyint(input * kInvLn2));
...
private:
    ...
    static std::vector<VxmLutEntry> make_table(
        float input_min,
        float segment_width,
        std::size_t count,
        Function function)
        ...
        for (std::size_t index = 0;
            ...
            const auto x0 =
                ...
                + static_cast<float>(index)
                    * segment_width;
            const auto y0 = function(x0);
            result.push_back(
                VxmLutEntry::from_float(
                    (function(
                         x0 + segment_width)
                     - y0)
                        / segment_width,
