...
public:
    ...
    struct TimingSnapshot {
        std::size_t cycle{0};
        std::size_t vxm_issues{0};
        std::size_t mem_issues{0};
        std::size_t mxm_load_issues{0};
        std::size_t mxm_dequant_issues{0};
        std::size_t mxm_compute_issues{0};
        std::size_t sxm_transpose_issues{0};
        std::size_t sxm_permute_issues{0};

        std::size_t total_issues() const noexcept
        {
            return vxm_issues + mem_issues + mxm_load_issues
                + mxm_dequant_issues + mxm_compute_issues
                + sxm_transpose_issues + sxm_permute_issues;
        }
    };
    ...
    {
        auto result = TimingSnapshot{};
        result.cycle = cycle_ == 0 ? 0 : cycle_ - 1;
        const auto issued = [](const auto& queues) {
            return static_cast<std::size_t>(std::count_if(
                queues.begin(), queues.end(), [](const auto& queue) {
                    const auto action = queue.last_trace().action;
                    return action == IcuQueueAction::FunctionalIssue
                        || action == IcuQueueAction::RepeatIssue;
                }));
        };
        result.vxm_issues = issued(vxm_queues_);
        result.mem_issues = issued(mem_queues_);
        result.mxm_load_issues = issued(mxm_load_queues_);
        result.mxm_dequant_issues = issued(mxm_dequant_queues_);
        result.mxm_compute_issues = issued(mxm_compute_queues_);
        result.sxm_transpose_issues = issued(sxm_transpose_queues_);
        result.sxm_permute_issues = issued(sxm_permute_queues_);
        return result;
    }
