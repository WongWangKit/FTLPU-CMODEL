...
private:
    ...
    static void print_instruction(std::ostream& os, const MemInstruction& instruction)
    {
        ...
        case MemOpcode::Write:
            ...
            break;
        case MemOpcode::ReadWrite:
            os << " read=b" << instruction.address.bank()
               << ":w" << instruction.address.word()
               << " rs=" << instruction.stream
               << " write=b" << instruction.write_address.bank()
               << ":w" << instruction.write_address.word()
               << " ws=" << instruction.write_stream;
            break;
        case MemOpcode::Gather:
        case MemOpcode::Scatter:
