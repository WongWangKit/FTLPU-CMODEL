...
inline EncodedSxmInstruction encode_sxm_instruction(const SxmInstruction& instruction)
{
    ...
    constexpr std::size_t kReservedLaneMapOffset = kDstOffset + kMaxStreams * 6;
    constexpr std::size_t kPermuteMapOffset =
        kReservedLaneMapOffset + hw::kLanesPerTile * 4;
    ...
    if (instruction.opcode != SxmOpcode::Transpose
        ...
        throw std::invalid_argument("SXM opcode is not supported by the physical slice");
    }
    detail::require_unsigned_fit(
...
inline SxmInstruction decode_sxm_instruction(const EncodedSxmInstruction& encoded)
{
    ...
    if (instruction.opcode != SxmOpcode::Transpose
        && instruction.opcode != SxmOpcode::Permute) {
        throw std::logic_error("encoded SXM opcode is not supported by the physical slice");
    }
    if (detail::read_bits(encoded.words, 2, 4) != 0) {
        throw std::logic_error("encoded SXM instruction uses removed shift control bits");
    }
    ...
    for (std::size_t bit = kReservedLaneMapOffset;
         bit < kPermuteMapOffset;
         ++bit) {
        if (detail::read_bits(encoded.words, bit, 1) != 0) {
            throw std::logic_error("encoded SXM instruction uses removed lane-map bits");
        }
    }
    for (std::size_t lane = 0; lane < SxmInstruction::kTotalLanes; ++lane) {
        const auto source =
