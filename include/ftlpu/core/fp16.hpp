...
// IEEE-754 binary16 storage with deterministic round-to-nearest-even
// conversion on hosts without a native half type. Migrated from the
// f621374 workload reference without importing its incompatible VXM.
class Fp16 {
public:
    ...
    static Fp16 from_float(float value) noexcept
    {
        ...
        const auto exponent =
            static_cast<int>((bits >> 23) & 0xffu) - 127 + 15;
        const auto mantissa = bits & 0x7fffffu;

        if (((bits >> 23) & 0xffu) == 0xffu) {
            const auto payload =
                mantissa == 0 ? 0u : ((mantissa >> 13) | 1u);
            return Fp16(static_cast<std::uint16_t>(
                sign | 0x7c00u | payload));
        }
        if (exponent >= 31) {
    ...
    {
        const auto sign =
            static_cast<std::uint32_t>(bits_ & 0x8000u) << 16;
...
inline float round_to_fp16(float value) noexcept
{
