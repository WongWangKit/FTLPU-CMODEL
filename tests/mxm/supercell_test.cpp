...
int main()
{
    ...
    for (std::size_t cycle = 0;
         cycle < ftlpu::MxmSupercell::kWavefrontStages;
         ++cycle) {
        supercell.tick(log);
        const auto& outputs = supercell.outputs();
        if (cycle < ftlpu::hw::kMxmSupercellRows - 1) {
            assert(outputs.empty());
            continue;
        }
        const auto column = cycle - (ftlpu::hw::kMxmSupercellRows - 1);
    ...
    for (std::size_t cycle = 0;
         cycle < ftlpu::hw::kMxmSupercellRows;
         ++cycle) {
        bf16_supercell.tick(log);
    }
    ...
    // Latency is 15 cycles, but the diagonal pipeline accepts one activation
    // vector every cycle.  Eight consecutive tokens therefore produce eight
    // column results in the same steady-state cycle.
    ftlpu::MxmSupercell throughput;
    throughput.set_input(full_input());
    throughput.issue(ftlpu::MxmInstruction::IW(0));
    throughput.tick(log);
    std::size_t result_count = 0;
    std::size_t peak_results = 0;
    constexpr std::size_t kTokens = 8;
    for (std::size_t cycle = 0;
         cycle < kTokens + ftlpu::MxmSupercell::kWavefrontStages - 1;
         ++cycle) {
        if (cycle < kTokens) {
            throughput.set_activation_input(
                activation_input(0.5f + static_cast<float>(cycle)), 0);
        }
        throughput.tick(log);
        result_count += throughput.outputs().size();
        peak_results = std::max(peak_results, throughput.outputs().size());
    }
    assert(result_count == kTokens * ftlpu::hw::kMxmSupercellColumns);
    assert(peak_results == ftlpu::hw::kMxmSupercellColumns);

    // The inactive peer buffer remains writable while buffer 0 has a wave in
    // flight, but overwriting buffer 0 itself is a hardware scheduling error.
    ftlpu::MxmSupercell ping_pong;
    ping_pong.set_input(full_input());
    ping_pong.issue(ftlpu::MxmInstruction::IW(0));
    ping_pong.tick(log);
    ping_pong.set_activation_input(activation_input(), 0);
    ping_pong.tick(log);
    ping_pong.set_input(full_input());
    ping_pong.issue(ftlpu::MxmInstruction::IW(1));
    ping_pong.tick(log);
    ping_pong.set_input(full_input());
    ping_pong.issue(ftlpu::MxmInstruction::IW(0));
    caught = false;
