...
ftlpu::MxmControlSlice::WeightInput half_row_input(
    std::size_t tile,
    ...
    bool upper)
{
    ...
    const auto first = upper
        ? ftlpu::hw::kMxmSupercellColumns / 2
        : std::size_t{0};
    const auto end = first + ftlpu::hw::kMxmSupercellColumns / 2;
    for (std::size_t lane = 0; lane < ftlpu::hw::kLanesPerTile; ++lane) {
        for (std::size_t stream = first; stream < end; ++stream) {
            input[lane][stream] = ftlpu::MxmArray::Supercell::InputWord {
                expected_weight(tile, supercell_column, lane, stream),
                stream + 1 == end,
            };
        }
    }
...
int main()
{
    ...
    // The fixed background window sends lower and upper halves in consecutive
    // waves. They have independent horizontal shift state, so neither wave
    // may replay and overwrite the other half while it advances.
    auto background_array = std::make_unique<ftlpu::MxmArray>();
    ftlpu::MxmControlSlice background_control(*background_array);
    constexpr std::size_t kBackgroundBuffer = 0;
    for (std::size_t pulse = 0;
         pulse < ftlpu::hw::kMxmSupercellsPerPlane; ++pulse) {
        background_control.issue_south(ftlpu::MxmControlInstruction::IW(
            kBackgroundBuffer,
            ftlpu::MxmWeightLoadMode::BackgroundLowerHalf));
    }
    for (std::size_t pulse = 0;
         pulse < ftlpu::hw::kMxmSupercellsPerPlane; ++pulse) {
        background_control.issue_south(ftlpu::MxmControlInstruction::IW(
            kBackgroundBuffer,
            ftlpu::MxmWeightLoadMode::BackgroundUpperHalf));
    }
    auto background_provider = [&background_control](
        std::size_t tile,
        const ftlpu::MxmControlInstruction& instruction) {
        auto pulse = background_control.cycle() - tile;
        const auto upper = instruction.weight_load_mode
            == ftlpu::MxmWeightLoadMode::BackgroundUpperHalf;
        if (upper) pulse -= ftlpu::hw::kMxmSupercellsPerPlane;
        const auto target_column =
            ftlpu::hw::kMxmSupercellsPerPlane - 1 - pulse;
        return half_row_input(tile, target_column, upper);
    };
    for (std::size_t cycle = 0;
         cycle < 3 * ftlpu::hw::kMxmSupercellsPerPlane - 1;
         ++cycle) {
        background_control.tick(log, background_provider, false);
    }
    for (std::size_t tile = 0;
         tile < ftlpu::hw::kMxmSupercellsPerPlane; ++tile) {
        for (std::size_t column = 0;
             column < ftlpu::hw::kMxmSupercellsPerPlane; ++column) {
            if (!require(background_control.loaded_cell(
                    kBackgroundBuffer, tile, column),
                    "background half waves should complete every cell")) {
                return 1;
            }
            for (std::size_t lane = 0;
                 lane < ftlpu::hw::kLanesPerTile; ++lane) {
                for (std::size_t stream = 0;
                     stream < ftlpu::hw::kMxmSupercellColumns; ++stream) {
                    if (!require(background_array->weight(
                            kBackgroundBuffer, tile, column, lane, stream)
                            == expected_weight(
                                tile, column, lane, stream),
                            "background half-wave weight mismatch")) {
                        return 1;
                    }
                }
            }
    ...
    constexpr std::size_t kOutputStream = 30;
    parallel_control.issue_south(ftlpu::MxmControlInstruction::IW(kBuffer));
    parallel_control.issue_south(ftlpu::MxmControlInstruction::Compute(kBuffer, kOutputStream));
