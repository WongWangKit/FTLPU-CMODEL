...
int main()
{
    ...
    assert(distributed[0]
        == 100 + static_cast<std::int32_t>(ftlpu::hw::kLanesPerTile - 1));
    ...
    assert(north[0][0] == 1);
    assert(north[0][ftlpu::hw::kLanesPerTile - 2]
        == static_cast<std::int32_t>(ftlpu::hw::kLanesPerTile - 1));
    assert(north[0][ftlpu::hw::kLanesPerTile - 1]
        == static_cast<std::int32_t>(ftlpu::hw::kLanesPerTile));
    assert(north[ftlpu::hw::kTileRows - 2]
                [ftlpu::hw::kLanesPerTile - 1]
        == static_cast<std::int32_t>(
            (ftlpu::hw::kTileRows - 1) * ftlpu::hw::kLanesPerTile));
    assert(north[ftlpu::hw::kTileRows - 1][0]
        == static_cast<std::int32_t>(
            (ftlpu::hw::kTileRows - 1) * ftlpu::hw::kLanesPerTile + 1));
    assert(north[ftlpu::hw::kTileRows - 1]
                [ftlpu::hw::kLanesPerTile - 1]
        == -1);
    ...
    assert(south[0][2] == 0);
    assert(south[1][0]
        == static_cast<std::int32_t>(ftlpu::hw::kLanesPerTile - 2));
    assert(south[ftlpu::hw::kTileRows - 1]
                [ftlpu::hw::kLanesPerTile - 1]
        == static_cast<std::int32_t>(
            ftlpu::hw::kTileRows * ftlpu::hw::kLanesPerTile - 3));
    ...
    assert(permuted[0][0]
        == static_cast<std::int32_t>(
            ftlpu::hw::kTileRows * ftlpu::hw::kLanesPerTile - 1));
    assert(permuted[0][ftlpu::hw::kLanesPerTile - 1]
        == static_cast<std::int32_t>(
            (ftlpu::hw::kTileRows - 1) * ftlpu::hw::kLanesPerTile));
    assert(permuted[ftlpu::hw::kTileRows - 1]
                [ftlpu::hw::kLanesPerTile - 1]
        == 0);
    ...
    sxm.complete_cycle();
    assert(evaluation.produced[ftlpu::hw::kLanesPerTile]);
    assert(evaluation.outputs[ftlpu::hw::kLanesPerTile][3]->data == 300);
    assert(evaluation.outputs[2 * ftlpu::hw::kLanesPerTile - 1][3]->data
        == 300 + static_cast<std::int32_t>(ftlpu::hw::kLanesPerTile - 1));

    auto chained_map = ftlpu::Distribute16::identity_map();
    chained_map[0] = 3;
    sxm.issue(ftlpu::SxmInstruction::Distribute(
        ftlpu::SxmStreamId {ftlpu::hw::kLanesPerTile},
    ...
    assert(fabric.cell(1, 0, 0, east1).data
        == ftlpu::hw::kLanesPerTile - 1);
    assert(fabric.cell(
               1, ftlpu::hw::kTileRows - 1, 0, east1).data
    ...
    // Static-quantized VXM/MEM traffic uses the same block machinery with a
    // single INT8 byte-plane. A one-stream Permute is explicitly classified
    // as block routing (rather than the legacy global vector operation).
    ftlpu::StreamRegisterFabric int8_fabric(2);
    ftlpu::SxmSlice int8_sxm(
        ftlpu::SxmStreamPortMap::SameDirection(0, 1));
    const auto int8_in = ftlpu::StreamId::East(0);
    const auto int8_transposed = ftlpu::StreamId::East(1);
    const auto int8_routed = ftlpu::StreamId::East(2);
    const auto int8_transpose_instruction =
        ftlpu::SxmInstruction::Transpose(
            {{int8_in.packed()}},
            {{int8_transposed.packed()}},
            1);
    for (std::size_t row = 0;
        ...
        int8_sxm.issue(int8_transpose_instruction);
        int8_fabric.begin_cycle();
        int8_sxm.evaluate(int8_fabric);
        int8_fabric.commit_cycle();
    }
    int8_fabric.begin_cycle();
    int8_sxm.evaluate(int8_fabric); // full registered boundary cycle
    int8_fabric.commit_cycle();

    for (std::size_t column = 0;
         column < ftlpu::hw::kLanesPerTile;
         ++column) {
        int8_sxm.issue(ftlpu::SxmInstruction::Permute(
            {{int8_transposed.packed()}},
            {{int8_routed.packed()}},
            ftlpu::Permute320::identity_map(),
        ...
        for (std::size_t lane = 0;
             lane < ftlpu::hw::kLanesPerTile;
             ++lane) {
            assert(int8_fabric.cell(
                       1, 0, lane, int8_routed).data
                == static_cast<std::uint8_t>(
                    lane * ftlpu::hw::kLanesPerTile + column));
        }
    }

    // A parallel FP16 beat carries all low/high row pairs. Transpose produces
    // the canonical lane-count square block. Permute exchanges complete
    // blocks across superlanes on the following cycle.
    ftlpu::StreamRegisterFabric parallel_fabric(2);
    ftlpu::SxmSlice parallel_sxm(ftlpu::SxmStreamPortMap::SameDirection(0, 1));
    constexpr auto kParallelStreams =
        2 * ftlpu::hw::kLanesPerTile;
    constexpr auto kParallelInternalBase =
        2 * kParallelStreams <= ftlpu::hw::kStreamsPerDirection
        ? kParallelStreams : 0;
    ...
    for (std::size_t cycle = 0; cycle < ftlpu::hw::kTileRows; ++cycle) {
        ...
        if (cycle == 0) {
            parallel_sxm.issue(ftlpu::SxmInstruction::Transpose(
                stream_range(0, kParallelStreams),
                stream_range(kParallelInternalBase, kParallelStreams)));
        }
