...
OperatorResult run_softmax()
{
    ...
    const auto normalize_wave = [&](std::size_t wave) {
    ...
    for (std::size_t wave = 0; wave < 2; ++wave) {
        ...
        // The four reciprocal results live in the four C3 local scalar
        // registers.  Normalize this wave before the next wave overwrites
        // those deliberately small registers.
        normalize_wave(wave);
    }
