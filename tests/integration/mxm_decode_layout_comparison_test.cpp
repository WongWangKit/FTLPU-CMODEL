...
std::size_t program_native(Schedule& schedule)
{
    ...
    for (std::size_t reduction = 0; reduction < kK / kNativeBlock; ++reduction) {
        ...
        // A buffer remains live from its four-cycle activation broadcast
        // through the complete 21-cycle array wave.  Two ping-pong buffers
        // therefore permit a new reduction every ceil((4+21)/2)=13 cycles.
        constexpr auto kNativeIssueInterval =
            (kNativeStages + ftlpu::hw::kMxmSupercellsPerPlane + 1) / 2;
        phase += kNativeIssueInterval;
    }
    return schedule.end_cycle() + 8;
}
